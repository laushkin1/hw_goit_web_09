...
def save_quotes(quotes):
    with open("quotes.json", "w", encoding="utf-8") as fl:
        json.dump(quotes, fl, ensure_ascii=False)
...
def get_authors():
    ...
    while True:
        ...
        for author_div in soup.find_all("div", class_="quote"):
            author = {"fullname": author_div.find("small", class_="author").get_text()}
            about = (
                url + author_div.find("small", class_="author").find_next("a")["href"]
            )
...
def save_authors(authors):
    with open("authors.json", "w", encoding="utf-8") as fl:
        json.dump(authors, fl, ensure_ascii=False)
